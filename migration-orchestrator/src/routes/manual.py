...
async def zoning_mirror(project: str, path: str):
    ...
    # no-cache: the inspector page is regenerated by the engine; revalidate so a fresh
    # .manual.html (new inspector JS) is picked up on reload instead of a stale browser copy.
...
async def zoning_set_namespace(project: str, request: Request) -> dict:
    """Set the one namespace prefix (a valid JCR/XML prefix: a letter then letters/digits)."""
    _require(project)
    body = await request.json()
    ns = (body.get("namespace") or "").strip()
    if not re.match(r"^[A-Za-z][A-Za-z0-9]*$", ns):
        raise HTTPException(status_code=400,
                            detail="invalid namespace (a letter, then letters/digits — e.g. custom)")
    p = _config_path(project)
    p.parent.mkdir(parents=True, exist_ok=True)
    cfg = {}
    if p.is_file():
