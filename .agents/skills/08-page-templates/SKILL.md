...
- **A shared region renders in the Page Builder edit canvas ONLY if its node has child content nodes.** An `AbsoluteArea` renders the region node's *children* as editable modules in edit mode; a region whose node has **zero children** (e.g. a nav whose menu is *derived* from the page tree, or a footer whose content was never created) renders **blank in edit mode** — it hits `TemplateNotFoundException: default` internally and is silently swallowed — even though the live render works fine. Symptom: "I don't see the nav/footer in the Page Builder." Fix: ensure every shared region has at least one editable child node. For nav, store the header CTA buttons as child `ns:ctaButton` nodes (this makes the nav both visible AND editable in edit mode); for footer, create the real social/CTA/legal child nodes. A region that has node properties but no children still renders blank in edit — children are what trigger edit-mode rendering. Verify with: `curl -b JSESSIONID=... .../cms/editframe/default/<lang>/sites/<site>/home.html | grep -ci <region-class>` (must be >0).
- **Shared regions must emit the theme's wrapper element/id, or their CSS never applies.** The imported theme scopes the header under `#header` and the **entire footer under `#footer`** (`#footer .bg-top-footer .grid-1` …). If the footer view renders just `.footerm2` with no `<footer><div id="footer">` ancestor, every footer rule misses and the footer is fully unstyled (social icons stacked as blue links, no layout). Transcribe the reference wrapper: `<footer><div id="footer"><div class="container"><div class="row">{component}…`. Check the theme CSS for the scoping prefix (`grep '#footer' main-theme.css`) and reproduce that exact id/structure. Same applies to any region (header→`#header`, footer→`#footer`).
