...
## Carousels (swiffy-slider) and list components in Page Builder

Two recurring traps with imported swiffy-slider carousels (partner-logo strips, hero sliders):
- **Items per view.** The slider needs a `slider-item-show{2..6}` class to show several items; the imported theme often pins `.<component> .slider-container{grid-auto-columns:100%}` (1 per view), which OVERRIDES `slider-item-showN`. Override it in CSS with `grid-auto-columns: 20% !important` (responsive: 50% mobile / 20% desktop). Without this only the first item shows and the rest sit off-screen (and lazy images never load).
- **Init + autoplay.** swiffy-slider loaded via `AddResources` does not reliably fire its `DOMContentLoaded` auto-init when injected late. Add an inline script that calls `window.swiffyslider.init()` (retry until defined), and put `data-slider-nav-autoplay="true"` `data-slider-nav-autoplay-interval="3500"` on the `.swiffy-slider` element for auto-scroll.

**Edit-mode rendering for any list/carousel of child nodes:** branch on `renderContext.isEditMode()` and render the children as a flat **bordered thumbnail grid** (not the live carousel). The live carousel hides all but a few slides, so in Page Builder editors can't see, click, or reorder most items — a bad management experience. A `display:grid; grid-template-columns:repeat(auto-fill,minmax(140px,1fr))` of bordered cells with capped image height makes every child individually editable.
