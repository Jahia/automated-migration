...
> **Automated gate:** `bash orchestration/probes/render-truth.sh <live_url>` is the machine version of "look at the render" — it loads the page headless, scrolls, and FAILS on broken images, content stuck at `opacity:0`, collapsed regions, and playerless video sections, saving a screenshot. Run it per page (and with `--edit` on a `/cms/editframe/...` URL) BEFORE claiming a page renders. It does not replace looking at the screenshot it produces — it guarantees you can't skip it.
