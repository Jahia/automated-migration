...
- Header CTA buttons need explicit **hover states** — without them the theme's generic `a:hover` can turn the whole button black or do nothing. Define hover on the wrapping `<a>` with a `transition`. **Use the theme `:root` tokens for the colors, never eyeballed hex** — the original's exact palette is in `main-theme.css :root` (`--secondary-color`, `--text-primary-color`, `--btn-radius`). Typical pattern: solid button = `bg:var(--secondary-color); color:#fff` → hover `bg:#fff; color:var(--secondary-color)`; outline button = `border:1px solid #fff; color:#fff` → hover `bg:#fff; color:var(--secondary-color)`. The theme often overrides the border-color, so the outline `border` may need `!important`.
