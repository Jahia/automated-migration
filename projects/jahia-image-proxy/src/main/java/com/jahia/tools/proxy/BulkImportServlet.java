...
/**
 * Generic servlet filter that processes bulk image-import requests by reading a JSON array
 * of image descriptors and importing each image into Jahia's JCR DAM.
 *
 * <p>Registers on two URL patterns for back-compatibility:
 * <ul>
 *   <li>{@code /modules/jahia-image-proxy/bulk-import} - canonical generic endpoint</li>
 *   <li>{@code /modules/sial/bulk-import} - legacy endpoint kept for existing callers</li>
 * </ul>
 *
 * <p>Request body (POST, {@code Content-Type: application/json}):
 * <pre>
 * [
 *   {"sourceUrl":"https://cdn.example.com/a.jpg","destPath":"/sites/mysite/files/images","filename":"a.jpg"},
 *   ...
 * ]
 * </pre>
 *
 * <p>Response: a JSON array with one entry per input - either {@code "success":true} or
 * {@code "success":false,"error":"..."}. Each entry is processed independently; a failure
 * on one does not abort the rest. Processing is capped at {@value #MAX_ENTRIES} entries
 * per request.
 *
...
public class BulkImportServlet extends AbstractServletFilter {
    ...
    private static final String ENDPOINT_GENERIC = "/modules/jahia-image-proxy/bulk-import";

    /** Legacy sial endpoint retained for back-compatibility with existing callers. */
    private static final String ENDPOINT_LEGACY  = "/modules/sial/bulk-import";
    ...
    /**
     * Intercepts requests matching either registered endpoint and delegates to the bulk import
     ...
     * each image independently, and writes a JSON array result.
     *
     ...
     *
     * @param json the JSON fragment with braces removed
    ...
    /**
     * Downloads the image at the given URL and returns its raw bytes.
     *
     ...
     * scheme and host of {@code sourceUrl} to satisfy origin-check policies for any domain.
     *
    ...
    private byte[] downloadImage(String sourceUrl) throws IOException {
        ...
        // Derive Referer from scheme+host so origin-check policies are satisfied for any domain.
