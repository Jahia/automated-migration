...
# Create news folder
echo ""
echo "Creating /contents/news folder..."
news_folder=$(curl -s -u "$JAHIA_USER" "$JAHIA_HOST/modules/graphql" \
  -H "Origin: $JAHIA_HOST" -H "content-type: application/json" \
  --data-raw '{"query":"mutation { jcr(workspace: EDIT) { mutateNode(pathOrId: \"/sites/sial-paris/contents\") { addChild(name: \"news\", primaryNodeType: \"jnt:contentFolder\") { uuid } } } }"}')
verify_response "$news_folder" "create news folder"
sleep 0.3
...
create_article() {
  ...
  article_resp=$(curl -s -u "$JAHIA_USER" "$JAHIA_HOST/modules/graphql" \
    -H "Origin: $JAHIA_HOST" -H "content-type: application/json" \
    --data-raw "{\"query\":\"mutation { jcr(workspace: EDIT) { mutateNode(pathOrId: \\\"/sites/sial-paris/contents/news\\\") { addChild(name: \\\"$name\\\", primaryNodeType: \\\"sialp:newsArticle\\\", properties: [{name: \\\"publishDate\\\", value: \\\"${date}T00:00:00.000+00:00\\\", type: DATE}, {name: \\\"category\\\", value: \\\"$category\\\"}]) { uuid } } } }\"}")
