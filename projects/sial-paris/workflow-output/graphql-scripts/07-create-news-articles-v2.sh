...
# Create news folder as jnt:folder inside contents
...
curl -s -u root:root \
  ...
  -d '{
    "query":"mutation { jcr { addNode(parentPathOrId: \"/sites/sial-paris/contents\", name: \"news\", primaryNodeType: \"jnt:folder\") { uuid } } }"
  }' > /dev/null
