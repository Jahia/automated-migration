...
curl -s -u root:root \
  ...
  -d '{
    "query":"mutation { jcr { addNode(parentPathOrId: \"/sites/sial-paris\", name: \"contents\", primaryNodeType: \"jnt:folder\") { uuid } } }"
  }' > /dev/null

# Create news folder
echo "Creating news folder..."
curl -s -u root:root \
  -H "Origin: http://localhost:8080" \
  -H "Content-Type: application/json" \
  -X POST http://localhost:8080/modules/graphql \
  -d '{
    "query":"mutation { jcr { addNode(parentPathOrId: \"/sites/sial-paris/contents\", name: \"news\", primaryNodeType: \"jnt:contentFolder\") { uuid } } }"
  }' > /dev/null
...
curl -s -u root:root \
  ...
  -X POST http://localhost:8080/modules/graphql \
  -d '{
    "query":"mutation { jcr { addNode(parentPathOrId: \"/sites/sial-paris/contents/news\", name: \"tendances-2026\", primaryNodeType: \"sialp:newsArticle\", properties: [{name: \"jcr:title\", value: \"Rapport tendances 2026 : les 5 grands enjeux de l&#39;alimentaire\", language: \"fr\"}, {name: \"category\", value: \"Tendances\", language: \"fr\"}, {name: \"publishDate\", value: \"2026-02-28T00:00:00.000+00:00\", type: DATE}, {name: \"excerpt\", value: \"Notre rapport annuel dévoile les grandes tendances qui vont façonner le marché alimentaire mondial en 2026.\", language: \"fr\"}, {name: \"body\", value: \"<p>Notre rapport annuel dévoile les grandes tendances.</p>\", language: \"fr\"}]) { uuid } } }"
  }'
