...
| `pixel.sh <project_path> <site> <lang> <pages\|@sitemap> [maxDiffPct]` | **PIXEL PARITY, per page, AT PAGE CREATION** — renders the live page AND its captured reference (base-href + browser headers so the capture-time DOM loads its real CSS/images; scroll + freeze + force-reveal normalize animation state) in headless Chromium and diffs pixel-by-pixel via canvas (no image deps). Saves `workflow-output/pixel/<slug>/{ref,local,diff}.png`. FAILS above threshold (default 2%; per-project + per-page overrides in `orchestration/content/<project>.pixel-config.json` incl. hideSelectors for third-party overlays). STRICT: a page with no reference fails. The exit criterion of every generated content-slice story |
