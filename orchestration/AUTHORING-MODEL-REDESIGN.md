...
| No `mainResource`; `jcrQuery`+`gridRow` emitted but **unwired** | `cnd_emit.py` always emits the `ns:jcrQuery`+`ns:gridRow` base types, and `load_main_resources.py` (ETL phase 2.5) *can* create mainResource content in a `jnt:contentFolder` and wire `jcrQuery.startNode` to it — but **`gen_plan.py` emits no mainResource step**, so every generated plan skips it. SingPost: 0 jcrQuery / 0 gridRow / 0 mainResource instances; layout via frozen `rawHtml` wrappers. (The older hand-authored `lesalondelaphoto-content.gen.plan.json` *does* wire it — a regression, not an absence.) | Restore into `gen_plan.py`: emit a `step_main_resources` (run `load_main_resources.py`) after media import; wire `segment2manifest` detail detection → `jmix:mainResource` types + contentFolder + a real `jcrQuery` listing; use `ns:gridRow` for page layout instead of skeleton wrappers |
...
| No **semantic mixin reuse** (CTA/media/SEO) | `cnd_emit.py` emits base marker mixins (`component`/`pageComponent`) correctly, but for FUNCTIONALITY it emits **21 generic numbered SLOT mixins** (`contribBody..9`, `contribImage..7`, `contribLabel..4`, `contribLink`) attached per-node by the loader. No `nsmix:cta`/`media`/`seo`. `j:linkType` is split between the `contribLink` slot AND 3 inline declarations. This dedupes field *definitions* but destroys semantic identity, shared labels (the raw `body3`), and shared render behavior — the opposite of luxe's `luxemix:cta` (one def + one label + one `CTA/index.tsx` view reused across types). | Emit the §10.1 reusable semantic mixins (`nsmix:cta`/`media`/`seo`), each with a resource-bundle label + a shared view; archetypes **compose** them (`> …, nsmix:cta, nsmix:media`). Retire the `contrib*` numbered-slot mixins and the inline `j:linkType`. |
