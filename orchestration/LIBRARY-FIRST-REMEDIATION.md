...
| **1b** | **Generic LAYOUT recognizer → `zone` = byte-exact `<Area>`** — the dominant lever, NO fidelity trade. **EMIT DONE + VALIDATED off-Jahia:** single-Area zones now emit a `layoutSection` (structured `layout:{open,close}` skin + parent-linked children, no HTML blob). Measured: discoverasr **608** conversions (zones 1325→717), contentful **28**; **0-DOM neutral** (contentful 0 diff, discoverasr no new drift vs baseline); **G1 passes** (0 shells, after teaching contribution.py that layoutSection is structural). Celled/columns deferred to 1c. **Deploy chain pending** (CND type + loader setting the skin + deployed view) — validatable only by deploying | `_recognize_layout` + emit + proxy branch **shipped** |
