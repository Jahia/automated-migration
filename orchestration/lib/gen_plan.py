...
def build_plan(p):
    ...
    module = [
        ...
        step("step_scaffold", "Headless scaffold (official templates)", "build",
             [f"Run: bash orchestration/lib/scaffold_module.sh {P} {MODULE}",
              f"PROBE: bash orchestration/probes/build.sh {PP}"],
             deps=["step_namespace_check"],
             outputs={"artifact_0": f"{PP}/package.json"}),
        step("step_assets", "Import CSS/JS/fonts/images into static/", "build",
             [f"Follow .agents/skills/03-import-assets/SKILL.md for {PP} (source: local-mirror assets).",
              f"PROBE: bash orchestration/probes/assets.sh {PP}"],
             deps=["step_scaffold"],
             inputs={**common, "skill": ".agents/skills/03-import-assets/SKILL.md"}),
        step("step_cnd_merge", "Install analyze CND + bundles into the module", "build",
             ...
             deps=["step_scaffold"]),
        step("step_components", "Implement ALL components (views from html-fragments)", "build",
             [f"Follow .agents/skills/07-implement-components/SKILL.md; one view per manifest type; HTML must match {PP}/workflow-output/html-fragments/ exactly; rawHtml renders its html property verbatim.",
              f"PROBE: bash orchestration/probes/components-all.sh {PP} {NS}",
              f"PROBE: bash orchestration/probes/contract.sh {PP} step_components"],
             deps=["step_cnd_merge", "step_assets"],
             inputs={**common,
                     "consumes": [f"{PP}/workflow-output/component-manifest.json",
                                  f"{PP}/workflow-output/html-fragments/",
                                  f"{PP}/workflow-output/views.json"]}),
        step("step_templates", "Layout + page templates + MainResource", "build",
             [f"Follow .agents/skills/08-page-templates/SKILL.md for {PP}.",
              f"PROBE: bash orchestration/probes/templates.sh {PP}",
              f"PROBE: bash orchestration/probes/contract.sh {PP} step_templates"],
             deps=["step_components"],
             inputs={**common, "skill": ".agents/skills/08-page-templates/SKILL.md"}),
    ...
    content = [
        ...
        step("step_content_load", "Load real content via MCP (components + passthrough)", "content",
             [f"Run: python3 orchestration/lib/load_content.py {P} {SITE} --clean",
              f"PROBE: python3 orchestration/probes/content-fidelity.py {SITE} {PP}"],
             deps=["step_mcp"]),
        step("step_publish_parity", "default vs live parity", "publish",
             [f"PROBE: bash orchestration/probes/publish-parity.sh {PP} {SITE} fr,en"],
             deps=["step_content_load"]),
        step("step_edit_frame", "Pages editable in jContent", "verify",
             [f"PROBE: bash orchestration/probes/edit-frame.sh {PP} {SITE} fr"],
