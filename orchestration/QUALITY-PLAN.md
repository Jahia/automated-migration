...
- **2026-07-03 — Orchestrator-driven run PROVEN (answers "the orchestrator wasn't running, how were you testing?").**
  Honest gap acknowledged: P2.5/P3 were driven MANUALLY (`run_plan.py` + ad-hoc bash + hand re-runs), NOT through the engine — so "deterministic gates generalise" was proven, "orchestrator-driven" was not. This closes it.
  - Launched the real engine (`migration-orchestrator` FastAPI `src.main:app` on :8001, which auto-attaches opencode on :4096, agent = deepseek-v4-flash) and POSTed a compact plan (`orchestration/plans/discoverasr-engine-demo.plan.json`) for the discoverasr holdout: connect → semantic → mirror-gate.
  - **The engine drove it end-to-end via opencode agents, per CONTROL-LOOP:** step_connect (PROBE verified) → step_semantic (agent ran `semantic_extract`, PROBE verified) → step_mirror_gate (agent ran `mirror_probe`, PROBE GREEN, agent returned `status:halt`) → engine PAUSED at a `mirror` gate (did NOT auto-advance) → operator `POST /gate {approve, reason}` (audited in the events table as `gate_decision`) → halted→done → run resumed. Two-level gate confirmed (step gate + epic `waiting_approval`).
  - **Two engine-integrity facts found:** (1) plan `repo_dir` MUST be the ABSOLUTE repo path — the verifier runs PROBEs with `cwd=repo_dir`; `"."` resolved to the engine's own dir → every PROBE `exit 127 No such file`. (2) engine binds `127.0.0.1`, not `localhost`.
  - Verdict: the SAME probes gate identically whether driven by the engine, `run_plan.py`, or by hand — so the P2.5/P3 results stand — AND the engine genuinely executes+gates+halts+resumes on API decisions. Wiring the FULL P2.5/P3 plan (contribution + G6 + roundtrip steps) into one engine run is the remaining end-to-end step.

- **2026-07-03 — P4 holdout, FULL loop engine-driven on discoverasr: reached the segmentation gate, which RED-BLOCKED on the FROZEN stability threshold (honest anti-overfit outcome).**
  The real engine (run_1783106607306) drove the full plan via opencode agents through step_connect → step_crawl (render) → step_localize (mirror gate PASS) → step_semantic → **step_segment (vision) → gate RED**. discoverasr's 8 pages are ONE template cluster (identical Ascott property/destination layout) → segment_probe sampled 1 representative (`en`): 14 components, coverage 68 %, **stability 0.733 < 0.8** (48 passthrough — a busy SPA home: cookie banner + hero + membership + many sections). The vision model segments this page ~27 % differently run-to-run.
...
- **2026-07-03 — P5 REGISTERED: engine-driven, LLM-assisted mode (`ASSIST-PLAN.md`).**
  Julian's directive: the committed orchestrator OWNS execution (DeepSeek + OVH Qwen vision via opencode); the LLM assistant intervenes only at pre-registered DECISION POINTS (strategy choice on gate-RED-after-retries: N=3 consensus, per-cluster 3-page sampling, A/B arms, heuristic swap, manual review) through new audited engine entry points (`decision_pending` state, `GET /decisions`, `POST /decide`). Frozen bars UNCHANGED (stability 0.8, coverage 50, G1 60/85, reconstruct 95, ground-truth 99) — only the segmentation stability ESTIMATOR changes, pre-registered as **protocol v2** in `ASSIST-PLAN.md §7` BEFORE any v2 measurement (mean-pairwise Jaccard over N=3 upfront runs; k=min(3,|cluster|) pages per cluster, majority verdict; gate = every cluster passes). P4 amendment (pre-measurement for the 20-page run): "zero manual interventions" now reads "zero OUT-OF-BAND interventions — all steering through the audited decision/gate API". Honesty note: discoverasr's pristine-holdout status is SPENT (0.733 known); protocol v2's holdout is the next unseen site. End state: **discoverasr 20 pages engine-driven to completion (M4)**, with M1 offline tooling validation, M2 engine decision smoke (also settles the `attempt:1/3` retry anomaly + the 600s step deadline vs ~27 vision calls), M3 optional acquia full-loop rehearsal.
  **Same-day amendment (pre-implementation), on Julian's direction:** (1) **Scope rules** — a second decision type (`ASSIST-PLAN.md §5`): editorial rulings like "exclude the cookie banner from the reconstruction", stored as pattern-keyed data (`scope-rules.json`: exclude | force_passthrough, selector/signature, reason) honored by segmentation (scoped DOM), partition (`excluded-by-rule` category, exactly-once preserved), G1 denominators, pixel gates (masks derived on BOTH sides, extending the committed-masks policy), and the content loader. Excluded share reported per page; >20% of rendered area flags Julian review. (2) **Intervention confinement** — the assistant acts only during CALIBRATION (analyze epics on the representative sample) plus two scheduled reviews (`step_model_review`, end-of-batch `step_exceptions_review`); every decision generalizes as a site-level pattern rule (page-scoped rules = Julian only); the BATCH phase (remaining 30–10,000 pages) is orchestrator-only, misfit pages queue to an exceptions report instead of escalating mid-batch. M4 acceptance now includes a confinement audit (every assistant decision timestamp inside calibration/review windows; zero assistant actions in batch). (3) **`claude_adjudicate` strategy** (Julian: "on utilise ta vision et ta réflexion pour trancher et aider à aller vers du vert") — if protocol v2 stays RED, the assistant adjudicates a component set from the N gated runs' overlays (real outline ids only, coverage floor kept); the page is GREEN-BY-ADJUDICATION, recorded distinctly, stability numbers still reported, downstream deterministic gates unchanged; ordered BEFORE the heuristic arm swap.

- **2026-07-04 — P5 IMPLEMENTED + M4 engine-driven run to 18/26, frozen on quota (full record: `ASSIST-PLAN.md §12-13`).**
  One night, engine-driven with heavy assistant co-engineering: the decision protocol went LIVE (audited scope-rule decision excluded the TrustArc banner → segmentation 5/5 pages 0.833–1.0 over the frozen 0.8; two `proceed` reviews; two reviewer overrules; one fidelity HALT approval with compose evidence). ALL analyze+module gates green with REAL numbers: mirror 0/0, G1 **73.0/85.8** (floors 60/85, was 50.3/76.3 before the vision-extraction bridge), G5 media **100%** (was 44%), links 100%, compose **20/20 byte-exact** (new pre-Jahia gate + drill-down component map), fidelity 100% content / 92.5–100 pixel, deploy bundle ACTIVE, site created (4th, license held), 20/20 pages in JCR. Run `run_1783115025242` died at `step_pages` on a DeepSeek narrative flake (3× missing JSON envelope with green probes) — fix committed (verifier: probes are the truth), engine not yet restarted, **resume procedure in ASSIST-PLAN §12**. Twelve generic finds fixed+committed (headline: engine retries NEVER worked — P4's "retried 3×" was false; the vision→extraction bridge was never wired; localizer URL-encoding; deploy credential shadowing; reviewer-overrule semantics; integrity belt reading Jahia GraphQL as plan-independent completeness check — Julian's ask). Honest verdict: an ENGINEERING run, not a confinement-clean graded run — that clean run is the remaining P5 proof. Thresholds unchanged throughout. LLM spend (ledger, standing report): DeepSeek 9 calls / 912k in / 70k out / 16.8M cache (~$0.075); OVH vision 27 calls / 262k in / 21.5k out; Claude subagents ~2.5–3M tokens (the real cost — see §13 Q2.1). The two questions (biggest fear: long-tail non-convergence; what Julian may not realize: LLM cost asymmetry, typed-vs-rawHtml structural editability, FR shell, run not confinement-clean) are answered in ASSIST-PLAN §13.
