...
30. **Lazy images: the crawl captures PRE-hydration HTML.** The real URL lives only in `data-src`/`data-srcset` (src empty/absent); the site's JS copies it to `src` on scroll — JS that runs in NEITHER the offline mirror NOR the JS-stripped Jahia render. `localize_site` must localise the data-* URL AND MATERIALISE it into `src`/`srcset` (measured: discoverasr 10→52 images rendered). Any image referenced only in a data-attr is invisible to a real visitor unless materialised.
...
33. **A pure client-rendered SPA (main content via runtime XHR) CANNOT be mirrored offline — and the mirror gate MUST refuse it.** discoverasr's header/footer are server-rendered but the entire body (hero, offers, property cards) is fetched from an API after load (`data-xhr`, `hiddenMain`) — absent from the HTML. Mirror-fidelity stays ~5% no matter what assets are captured. This is the correct anti-overfit boundary: never fake-migrate content the pipeline cannot faithfully represent. Detect it (thin `<main>` text + high `data-xhr`) and STOP at the mirror gate, honestly.
